...
function confirmSubmit() {
    ...
    if (!validation.isValid) {
        Utils.ui.showToast(validation.message, 'error');
        return;
    }
...
function collectFormData() {
    ...
    return {
        ...
        date: dateValue,
        weekday: Utils.format.weekday(dateValue),
...
function calculateAvgPriceValue() {
...
function calculateFoodRatioValue() {
...
function calculateSetRatioValue() {
    const food = getNumberValue('food-revenue');
    const set = getNumberValue('set-revenue');
    const total = food + set;
    ...
    summary.innerHTML = `
        <div class="preview-card">
            ...
            <div class="preview-row">
                ...
                <span class="preview-value">${Utils.format.currency(data.totalRevenue)}</span>
            </div>
            <div class="preview-row">
            ...
            <div class="preview-row">
                ...
                <span class="preview-value">${Utils.format.currency(data.avgPrice)}</span>
            </div>
            <div class="preview-row">
                <span class="preview-label">現金 / 刷卡</span>
                <span class="preview-value">${Utils.format.currency(data.cashAmount)} / ${Utils.format.currency(data.cardAmount)}</span>
            </div>
            ${data.remarks ? `
            <div class="preview-row">
...
function submitCheckout() {
    ...
    // 呼叫後端 API
    Utils.api.callGAS(App.config, 'submitCheckout', { rowData: rowData, staffName: formData.staff }, function(response) {
        App.state.isSubmitting = false;
        Utils.ui.closeModal();
        
        if (response.status === 'success') {
            Utils.ui.showToast('結帳資料已送出！', 'success');
            
            // 重設表單
            resetForm();
        } else {
            Utils.ui.showToast('送出失敗：' + (response.message || '未知錯誤'), 'error');
        }
        ...
        App.state.isSubmitting = false;
        Utils.ui.closeModal();
        Utils.ui.showToast('連線錯誤，請稍後再試', 'error');
